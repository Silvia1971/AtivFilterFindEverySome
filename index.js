...
function filterCarros() {
    ...
    var carrodisp = item.filter(carro => carro.qtd > 0);

    carrodisp.forEach(function(it)
    {
        text += "<li>";        
        text += "Id: " + it.id + " - ";
        text += "Marca: " + it.marca + " - ";
...
function someCarros(){
    var lista = document.getElementById("lista");
    var disp = item.some(carro => carro.qtd > 0);
    if (disp == true)
    {
        lista.innerHTML = "<b>Existem carros disponíveis</b>";
    }
    else
    {
        lista.innerHTML = "<b>Não existem carros disponíveis</b>";
    }

}

function filterValor(){
    var lista = document.getElementById("lista");
    var text = "";
    var carrodisp = item.filter(carro => carro.valorunit > 500);
...
function findCarros(){
    var lista = document.getElementById("lista");
    var txtPesq = document.getElementById("txtPesq");
    
    
    var carrodisp = item.filter(carro => carro.marca === txtPesq.value);
    //alert(carrodisp.marca);
